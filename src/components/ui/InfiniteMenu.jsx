...
// ─── Fetch image as blob to avoid canvas taint ───────────────────────────────
...
// ─── Helper: draw a rounded-rect skill card on canvas ────────────────────────
function drawSkillCard(ctx, x, y, cellSize, img, title) {
  ...
  ctx.save();
  ctx.beginPath();
  ctx.roundRect(x + pad * 0.5, y + pad * 0.5, cellSize - pad, cellSize - pad, radius);
  const isDarkCard = document.documentElement.classList.contains('dark');
  ctx.fillStyle = isDarkCard ? '#111827' : '#e2e8f0';
  ctx.fill();
  ...
  if (img) {
    const logoSize = innerSize * 0.52;
    const logoX = x + cellSize / 2 - logoSize / 2;
    const logoY = y + pad + innerSize * 0.08;
    ctx.save();
    ctx.beginPath();
    ctx.arc(x + cellSize / 2, logoY + logoSize / 2, logoSize / 2, 0, Math.PI * 2);
    ctx.clip();
    ctx.drawImage(img, logoX, logoY, logoSize, logoSize);
    ctx.restore();
  } else {
    // Colorful initial badge as fallback
    const colors = ['#3b82f6','#10b981','#f59e0b','#ef4444','#8b5cf6','#06b6d4','#f97316'];
    const colorIndex = (title || '').charCodeAt(0) % colors.length;
    const cx = x + cellSize / 2;
    const cy = y + pad + innerSize * 0.34;
    const r = innerSize * 0.26;
    ctx.save();
    ctx.beginPath();
    ...
    ctx.fillText((title || '?')[0].toUpperCase(), cx, cy);
    ctx.restore();
  }

  // Skill name at bottom
  const fontSize = Math.round(cellSize * 0.1);
  ctx.save();
  ctx.font = `bold ${fontSize}px "Inter", "Segoe UI", sans-serif`;
  ctx.fillStyle = isDarkCard ? '#ffffff' : '#111827';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(title || '?', x + cellSize / 2, y + pad + innerSize * 0.78, innerSize * 0.9);
  ctx.restore();
}
...
class InfiniteGridMenu {
  ...
  // ─── FIXED: fetch as blob to avoid canvas taint ─────────────────────────────
  #initTexture() {
    const gl = this.gl;
    ...
    // Fill base background & upload placeholder immediately
    ...
    // Fetch each image as a blob (avoids canvas taint from cross-origin)
    Promise.all(this.items.map(item => fetchImageAsBlob(item.image)))
      .then(images => {
        images.forEach((img, i) => {
          const x = (i % this.atlasSize) * cellSize;
          const y = Math.floor(i / this.atlasSize) * cellSize;
          drawSkillCard(ctx, x, y, cellSize, img, this.items[i]?.title);
        });
